...
//  SERVICES
//
//  Created by Mohammed Abdullah on 15/04/1443 AH.
//
...
class ViewController: UIViewController {
    ...
    let urISession : URLSession = URLSession(configuration: .default)
    ...
    func getPetDataFromWeb() {
        ...
        let dataTask = urISession.dataTask(with: urlRequest, completionHandler: { data, response, error in
    ...
    func getPetImageFromWebDownloadTask (pet : Pet) {
        ...
        let downloadTask = urISession.downloadTask(with: imageURL, completionHandler: { localURL , response, error in
